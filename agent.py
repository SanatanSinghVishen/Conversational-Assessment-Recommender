import json
import re
from retriever import retrieve, _meta as catalog_items
...
def retrieve_specific_items(names: list[str]) -> list[dict]:
    """Retrieve catalog items by exact or fuzzy name match."""
    results = []
    ...
    for name in names:
        ...
        for item in catalog_items:
            ...
            # Fallback: semantic search for the name
...
async def process_chat(messages: list[dict]) -> dict:
    ...
    # --- PHASE 1: First LLM call to get intent + retrieval query ---
    phase1_prompt = build_system_prompt(
        catalog_context="",  # no catalog yet — just getting the intent
    ...
    if turn_cap_reached and intent != "recommend":
        intent = "recommend"
        # If we didn't get a retrieval query, build one manually
        if not retrieval_query:
            retrieval_query = build_retrieval_query(messages)

    # --- PHASE 2: Retrieve if needed ---
    retrieved_items = []
    if intent in ("recommend", "refine", "compare") and retrieval_query:
        retrieved_items = retrieve(retrieval_query, top_k=15)

    # --- PHASE 3: For recommend/refine, no second LLM call needed ---
    # The retrieved items ARE the recommendations (filtered to top 10).
    # For compare, we do a grounded second call to explain the difference.
    if intent == "compare" and retrieved_items:
        ...
        recommendations = []  # comparisons don't produce a shortlist
    else:
        ...
        "recommendations": [
            {
                "name": r.get("name", "N/A"), 
                "url": r.get("link", ""), 
                "test_type": ", ".join(r.get("keys", [])) if r.get("keys") else "N/A"
            }
